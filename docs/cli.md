...
Upon installation of `tgl`, it exposes the command line interface under binary `tgl`.
...
!!! warning "Virtualenv and other modifications of Python environment"
    As the `tgl` binary depends on the Python's environment, any action which would modify this environment, such as
    usage of virtualenv etc. might disable access to this binary. It depends on which environment you have installed `tgl` in.
...
`tgl` supports themes in order to be readable in all terminal settings. You can configure it as part of config file
...
`tgl` supports commands/options completion for `bash`, `zsh`, `PowerShell` and `fish` shells.

To enable the completion run `tgl config completion install`.

This command will install completion based on your current shell, it mostly consist of adding
`_TGL_COMPLETE` variable to your `rc` file. If you want to inspect details you can
run `tgl config completion show`.

### Date/time formats

Important part of the CLI is parsing of times & dates. `tgl` uses in background of `dateutil` library and it best-effort
...
* **TGL_CONFIG** - Defines which config file should be used. Should be absolute path. By default `~/.tglrc` is used.
